...
use Doctrine\Orient\ODM\Mapper\Hydration;
use Doctrine\Orient\ODM\Mapper\LinkTracker;
use Doctrine\Orient\Exception\Document\NotFound as DocumentNotFoundException;
use Doctrine\Orient\Formatter\CasterInterface as CasterInterface;
use Doctrine\Orient\Formatter\Caster;
use Doctrine\Orient\Contract\Formatter\Inflector;
use Doctrine\Orient\Filesystem\Iterator\Regex as RegexIterator;
use Doctrine\Orient\ODM\Mapper\Annotations\Property as PropertyAnnotation;
use Doctrine\Orient\Contract\Formatter\String as StringFormatterInterface;
use Doctrine\Orient\Formatter\String as StringFormatter;
use Doctrine\Orient\Contract\ODM\Mapper\Annotations\Reader as AnnotationreaderInterface;
use Doctrine\Orient\Exception\ODM\OClass\NotFound as ClassNotFoundException;
use Doctrine\Orient\Exception\Casting\Mismatch;
use Doctrine\Orient\ODM\Mapper\Annotations\Reader;
...
class Mapper
{
    ...
    const ANNOTATION_PROPERTY_CLASS = 'Doctrine\Orient\ODM\Mapper\Annotations\Property';
    const ANNOTATION_CLASS_CLASS    = 'Doctrine\Orient\ODM\Mapper\Annotations\Document';
     ...
     *
     * @param   string   $class
     * @return  Doctrine\Orient\ODM\Mapper\Class
     */
     ...
     * classes mapped for the Doctrine\Orient ODM.
     *
     * @return array
     */
     ...
     * @return Doctrine\Orient\ODM\Mapper\Property
     */
    ...
    /**
     * Takes an Doctrine\Orient JSON object and finds the class responsible to map that
     * object.
     ...
     * @throws  Doctrine\Orient\Exception\Document\NotFound
     */
     ...
     *
     * @param   Doctrine\Orient\ODM\Mapper\Annotations\Property  $annotation
     ...
     * @param   string $OClass
     * @return  string
     * @throws  Doctrine\Orient\Exception\ODM\OClass\NotFound
     */
    ...
    public function {$refMethod->getName()}($parametersAsString) {
        ...
        if (!is_null(\$parent)) {
            if (\$parent instanceOf \Doctrine\Orient\ODM\Proxy\AbstractProxy) {
                return \$parent();
            }
...
namespace Doctrine\Orient\Proxy$namespace;
...
EOT;
    ...
    protected function getProxyClass($class)
    {
        ...
        $proxyClassFQN = "Doctrine\Orient\Proxy" . $class;
    ...
    protected function mapProperty($document, $property, $value, PropertyAnnotation $annotation, LinkTracker $linkTracker)
    {
        ...
        if (method_exists($document, $setter)) {
            ...
            if ($refProperty->isPublic()) {
                ...
                throw new Exception(
                    sprintf("%s has not method %s: you have to added the setter in order to correctly let Doctrine\Orient hydrate your object ?",
                    get_class($document),
                    $setter)
                );
            }
     ...
     * @throws  Doctrine\Orient\Exception
     */
